...
class SyncSwap(DEX):
    ...
    async def add_liquidity(self):
        ...
        self.client.logger.info(
            f'{self.client.info} SyncSwap | Add liquidity to SyncSwap USDC/ETH pool: {amount_from_settings} ETH')

        token_a_address, token_b_address = SCROLL_TOKENS['ETH'], SCROLL_TOKENS['USDC']
    ...
    async def withdraw_liquidity(self):
        self.client.logger.info(
            f'{self.client.info} SyncSwap | Withdraw liquidity from SyncSwap')
