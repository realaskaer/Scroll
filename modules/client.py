...
class Client:
    ...
    async def get_auto_amount(self, token_name_search:str = None) -> [str, float, int]:
        ...
        wallet_balance = {k: await self.get_token_balance(k, False) for k, v in SCROLL_TOKENS.items()}
        ...
        if sum(valid_wallet_balance.values()) > self.min_amount_eth_on_balance * eth_price:
            ...
            token_names_list = list(filter(lambda token_name: token_name != from_token_name, SCROLL_TOKENS.keys()))
    ...
    async def get_token_balance(self, token_name: str = 'ETH', check_symbol: bool = True) -> [float, int, str]:
        if token_name != 'ETH':
            contract = self.get_contract(SCROLL_TOKENS[token_name])
